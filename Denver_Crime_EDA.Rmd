...
**You'll need to use ggmap::register_google to enable Google Maps for geo-plotting. Get an API Key here:  https://cloud.google.com/maps-platform/ **
...
Below we do some initial data cleaning and add some features. There are 104 cases where an offense is considered both a crime and traffic offense. The case when handles this issue when labeling.

```{r}
df %>% 
  select(IS_CRIME, IS_TRAFFIC, OFFENSE_CATEGORY_ID) %>% 
  group_by_all %>% 
  count %>% 
  arrange(desc(n))
...
```
Overall the dataset seems managable. There are about 201K observations.
```{r}
glimpse(df)
```
...
A good starting place is always a simple time-series plot of the data. This is a simple count of crime by day.
...
Here we can see the dataset dates back to 2014, but isn't really populated until 2017. Day-level granularity is quite noisy. Let's group by month/year to get a better handle of the trends.
...
Traffic is stable across the entire year, but it's crime that spikes during the summer. Guess we need to give out more free ice cream. There is nothing new here. We should expect traffic to remain stable. On first glance, we might be able to explain the spike during the summer months as free time for college/high shool students. Crime in general is typically attributed to younger males. Let's drill down even further:
...
The chart above shows the proportion of all crimes at each month. The proportions of offense categories remains pretty consistent over time - no significant seasonality. For example, if there happened to be a large increase in robbery, the volume of space at that point in time in the plot would expand. Overall, this plot helps to give a sense of how common any particular crime is compared to others.
...
* Burglary, auto theft, public disorder, and "other crimes against persons" are on the decline from the previous year.

* Motor-vehicle theft & aggravated assault are on the rise from the previous year.

* Everything else seems relatively consistent with the prior year. 
...
This assumes that the data collection process remained consistent year over year. If there was, say, some "higher quality reporting" of violent crime in 2018, that might skew the data even though the general crime rate remained constant.

To do this, we'll try to measure if the average daily incident rate was LOWER in 2017 than it was in 2018. Going back to day-level granularity gets us a larger sample size - better for testing. Let's start by making sure the distributions look appropriate (Gaussian) for testing.
...
ggplot(statsig, aes(x=as.factor(year), y=n)) +
  labs(title = "Box Plot of Daily Violent Incidents",
       x = "# of Incidents per Day") + 
...
Since the distributions look good, let's run a one-tailed test to see if the daily violent incident rate in 2017 was lower than 2018. The function we're using assumes unequal variances.

```{r}
print(t.test(n ~ year, data = statsig, alternative="less"))
```

P-value of less than 1% means that we accept our alternative hypothesis. 

We're 95% confident that the true difference in means is somewhere between $-\infty$ and -.28. How encouraging. 

What if we transformed the data by $log(n)$?
...
* District 3 leads in white collar crime, traffic, motor-vehicle theft, auto-theft, and burglary. Wow.

* District 2 leads in murder.
